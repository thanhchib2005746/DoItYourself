...
if (localStorage.cartList !== undefined) {
   ...
   var cartElement = '<div><img src="../images/NenThom/nenthomm01.jpg" alt="" class="giohang-hinhanh"><h1 class="giohang-tieude">Nến</h1><p class="giohang-gia">Giá</p><a href="#" class="giohang-xoa">Xóa</a></div>'
   ...
   for (let i = 0; i < listCartCourses.length; i++) {
      gioHangTieuDe[i].innerText = ListItems[listCartCourses[i]].Ten;
      gioHangGiaban[i].innerText = ListItems[listCartCourses[i]].Giaban;
      // gioHangGiaovien[i].innerText = courses[listCartCourses[i]].Giaovien;
      gioHangHinhAnh[i].src = ListItems[listCartCourses[i]].Hinhanh;
   ...
   for (let i = 0; i < listCartCourses.length; i++) {
      gioHangTongTien += Number.parseInt(ListItems[listCartCourses[i]].Giaban.replace(".", ""));
   }
   ...
   btnRemoveCarts.forEach(e => {
      e.addEventListener('click', (event) => {
         ...
         //* Trừ tổng tiền hàng
         gioHangTongTien -= Number.parseInt(ListItems[event.target.name].Giaban.replace(".", ""));
         document.querySelector('.giohang-tongtien').innerText = gioHangTongTien + "đ";
